...
var _default = function () {
  var _ref = _asyncToGenerator(function* (sessionToken, profile, providerAccount, options) {
    try {
      ...
      if (providerAccount.type === 'email') {
        ...
        if (userByEmail) {
          ...
          user = yield updateUserAndDispatchEvent(updateUser, events, _objectSpread(_objectSpread({}, profile), {}, {
        ...
        if (userByProviderAccountId) {
          if (isSignedIn) {
            if ("".concat(userByProviderAccountId.id) === "".concat(user.id)) {
              user = yield updateUserAndDispatchEvent(updateUser, events, profile, userByProviderAccountId);
              return {
                session,
                user,
                isNewUser
          ...
          if (isSignedIn) {
            ...
            user = yield updateUserAndDispatchEvent(updateUser, events, profile, user);
    ...
    function updateUserAndDispatchEvent(_x5, _x6, _x7, _x8) {
      return _updateUserAndDispatchEvent.apply(this, arguments);
    }

    function _updateUserAndDispatchEvent() {
      _updateUserAndDispatchEvent = _asyncToGenerator(function* (updateUserFunction, events, profile, originalUser) {
        var user = yield updateUserFunction(_objectSpread(_objectSpread({}, originalUser), profile));
